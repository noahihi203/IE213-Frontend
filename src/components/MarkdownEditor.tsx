...
export default function MarkdownEditor({
  ...
  const processUpload = async (file: File) => {
    if (!file.type.startsWith("image/"))
      return setLocalError("Chỉ hỗ trợ hình ảnh.");
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <ToolGroup>
            <ToolBtn
              ...
              title="Chèn Link (Tốt cho SEO Internal)"
            />
            <ToolBtn
              icon={<Table />}
              title="Chèn Bảng"
            ...
            {/* Image Upload Button */}
            <label
              className={`relative flex cursor-pointer items-center justify-center rounded p-1.5 text-slate-600 hover:bg-slate-200 hover:text-emerald-600 transition-colors ${isUploading ? "opacity-50 pointer-events-none" : ""}`}
              title="Tải ảnh lên (Tự động tạo Alt Text)"
            >
              {isUploading ? (
                <CircleNotch
                  size={18}
      ...
      >
        ...
        {(viewMode === "write" || viewMode === "split") && (
          <textarea
            ...
            className={` resize-none p-6 font-mono text-[15px] leading-relaxed text-slate-800 outline-none placeholder:text-slate-400 scrollbar-thin scrollbar-thumb-slate-200 bg-transparent ${
              viewMode === "split"
      ...
      <div className="flex items-center justify-between border-t border-slate-200 bg-white px-4 py-2 text-xs font-medium text-slate-500 shrink-0">
        ...
        <div>Cấu trúc thẻ SEO H1-H6 | Hỗ trợ Alt-Text Ảnh</div>
      </div>
    </div>
  );
}
