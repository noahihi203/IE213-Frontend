...
import {
  ...
  TextT,
  Link as LinkIcon,
  ArrowSquareOut, // <-- Added ArrowSquareOut
...
const LABELS = {
  create: {
    ...
    subtitle:
      "Trình soạn thảo chuyên nghiệp tích hợp công cụ chấm điểm SEO tự động.",
  },
  edit: {
    title: "Sửa Bài Viết",
...
export default function PostFormModal({
  ...
  const seoStats = useMemo(() => {
    ...
    if (kw) {
      if (tLower.includes(kw)) {
        score += 15;
        basicChecks.push({
          text: "Từ khóa chính xuất hiện trong Tiêu đề SEO",
          status: "good",
        });
      } else {
        basicChecks.push({
          text: "Thêm Từ khóa chính vào Tiêu đề SEO",
          status: "error",
        });
      }
      ...
      if (eLower.includes(kw)) {
        score += 10;
        basicChecks.push({
          text: "Từ khóa chính xuất hiện trong Mô tả Meta",
          status: "good",
        });
      } else {
        basicChecks.push({
          text: "Thêm Từ khóa chính vào Mô tả Meta",
          status: "error",
        });
      }

      const first10Percent = cLower.substring(
        0,
        Math.max(100, cLower.length * 0.1),
      );
      if (first10Percent.includes(kw)) {
        score += 10;
        basicChecks.push({
          text: "Từ khóa xuất hiện ở đầu bài viết",
          status: "good",
        });
      } else {
        basicChecks.push({
          text: "Nên đặt Từ khóa chính ở 10% nội dung đầu tiên",
          status: "warn",
        });
      }
    } else {
      basicChecks.push({
        text: "Chưa đặt Từ khóa chính (Focus Keyword)",
        status: "error",
      });
    }
    ...
    if (tLen >= 40 && tLen <= 60) {
      score += 10;
      basicChecks.push({
        text: `Độ dài tiêu đề tuyệt vời (${tLen}/60 ký tự)`,
        status: "good",
      });
    } else if (tLen > 0) {
      score += 5;
      basicChecks.push({
        text: "Tiêu đề quá ngắn hoặc quá dài (Tối ưu: 40-60 ký tự)",
        status: "warn",
      });
    } else {
      basicChecks.push({ text: "Thiếu tiêu đề bài viết", status: "error" });
    }
    ...
    if (eLen >= 120 && eLen <= 160) {
      score += 10;
      basicChecks.push({
        text: `Mô tả Meta chuẩn SEO (${eLen}/160 ký tự)`,
        status: "good",
      });
    } else if (eLen > 0) {
      score += 5;
      basicChecks.push({
        text: "Mô tả Meta chưa tối ưu (Tối ưu: 120-160 ký tự)",
        status: "warn",
      });
    } else {
      basicChecks.push({ text: "Thiếu mô tả Meta (Excerpt)", status: "error" });
    }
    ...
    if (wCount >= 600) {
      score += 15;
      advancedChecks.push({
        text: `Độ dài bài viết tuyệt vời (${wCount} từ)`,
        status: "good",
      });
    } else if (wCount >= 300) {
      score += 5;
      advancedChecks.push({
        text: `Độ dài tạm ổn (${wCount} từ). Đề xuất > 600 từ`,
        status: "warn",
      });
    } else {
      advancedChecks.push({
        text: "Nội dung quá ngắn (Cần > 300 từ)",
        status: "error",
      });
    }
    ...
    if (hasLinks) {
      score += 10;
      advancedChecks.push({
        text: "Tuyệt! Bài viết có chứa các liên kết (Links)",
        status: "good",
      });
    } else {
      advancedChecks.push({
        text: "Nên thêm Internal/External links để tăng SEO",
        status: "warn",
      });
    }
    ...
    if (hasHeadings) {
      score += 10;
      advancedChecks.push({
        text: "Đã sử dụng các thẻ Heading (H2, H3...) tốt",
        status: "good",
      });
    } else {
      advancedChecks.push({
        text: "Bài viết chưa sử dụng thẻ Heading (H2, H3)",
        status: "error",
      });
    }
    ...
    if (formData.coverImage) {
      score += 5;
      advancedChecks.push({
        text: "Đã thiết lập Ảnh đại diện bài viết",
        status: "good",
      });
    } else {
      advancedChecks.push({
        text: "Thiếu Ảnh đại diện (Gây giảm CTR mxh)",
        status: "error",
      });
    }

    if (formData.tags.length > 0 && formData.category) {
      score += 5;
      advancedChecks.push({
        text: "Đã thiết lập Danh mục và Thẻ Tag",
        status: "good",
      });
    } else {
      advancedChecks.push({
        text: "Cần chọn ít nhất 1 Danh mục và Tag",
        status: "warn",
      });
    }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("image/"))
      return setUploadError("Vui lòng chọn tệp hình ảnh hợp lệ.");
    if (file.size > MAX_FILE_SIZE)
      return setUploadError("Kích thước ảnh vượt quá 5MB.");
  ...
  const handleOpenFullPage = () => {
    ...
    if (mode === "create") {
      ...
      const idToRoute =
        postId || (formData as any).slug || (formData as any)._id;
                ...
                <div className="space-y-8">
                  ...
                  <div className="space-y-6 rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
                    <div>
                      <div className="mb-3 flex items-center justify-between">
                        ...
                        <span
                          className={`text-xs font-bold px-3 py-1 rounded-full ${seoStats.tLen >= 40 && seoStats.tLen <= 60 ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                        >
                          {seoStats.tLen} / 60
                        </span>
                      </div>
                    ...
                    <div>
                      <div className="mb-3 flex items-center justify-between">
                        ...
                        <span
                          className={`text-xs font-bold px-3 py-1 rounded-full ${seoStats.eLen >= 120 && seoStats.eLen <= 160 ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                        >
                          {seoStats.eLen} / 160
                        </span>
                      </div>
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col">
                    <div className="border-b border-slate-200 bg-slate-50 px-7 py-4">
                      <label className="text-[15px] font-bold text-slate-900 uppercase tracking-wider">
          ...
          <div className="w-[450px] shrink-0 border-l border-slate-200 bg-slate-50/50 flex flex-col">
            <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-slate-300 space-y-6">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm relative overflow-hidden">
                <div
                  className={`absolute top-0 left-0 w-full h-1.5 ${seoStats.bgClass.replace("bg-", "bg-").replace("50", "500")}`}
                ></div>
                <div className="mb-6 flex items-center justify-between">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-black uppercase ${seoStats.bgClass} ${seoStats.colorClass.replace("500", "700")}`}
                  >
                    {seoStats.score >= 80
                      ? "Tuyệt vời"
                      : seoStats.score >= 50
                  ...
                  <div className="relative flex h-32 w-32 items-center justify-center rounded-full bg-slate-50 shadow-inner">
                    <svg className="absolute inset-0 h-full w-full rotate-[-90deg]">
                      ...
                      <circle
                        ...
                        strokeDashoffset={
                          339.29 - (339.29 * seoStats.score) / 100
                        }
                        strokeLinecap="round"
                    ...
                    <div className="flex flex-col items-center">
                      <span
                        className={`text-4xl font-black ${seoStats.colorClass}`}
                      >
                        {seoStats.score}
                      </span>
                ...
                <div className="mb-6">
                  <label className="mb-2 block text-sm font-bold text-slate-700">
                    Từ khóa tập trung (Focus Keyword)
                  </label>
                  ...
                  {formData.coverImage ? (
                    ...
                    <div className="group relative flex h-48 w-full flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 transition-all hover:border-indigo-400 hover:bg-indigo-50/50">
                      ...
                      {isUploading ? (
                        ...
                        <UploadSimple
                          size={36}
                          className="text-slate-400 mb-3 transition-transform group-hover:-translate-y-1"
                          weight="duotone"
                        />
                      )}
